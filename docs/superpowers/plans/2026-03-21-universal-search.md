...
### Backend (new)
- `spaced-md.Server/Api/Endpoints/SearchEndpoints.cs` — search endpoint + DTOs
- `spaced-md.Server/Infrastructure/Data/Migrations/*_AddSearchVectors.cs` — generated by EF Core migration tool

### Backend (modify)
- `spaced-md.Server/Infrastructure/Data/AppDbContext.cs` — add SearchVector property config for Card, Deck, MarkdownFile
- `spaced-md.Server/Domain/Entities/Card.cs` — add SearchVector property
- `spaced-md.Server/Domain/Entities/Deck.cs` — add SearchVector property
- `spaced-md.Server/Domain/Entities/MarkdownFile.cs` — add SearchVector property
- `spaced-md.Server/Program.cs` — register `MapSearchEndpoints()`

### Frontend (new)
- `spaced-md.client/src/composables/useSearch.ts` — debounced search API + keyboard nav state
- `spaced-md.client/src/components/SearchResults.vue` — grouped results dropdown

### Frontend (modify)
- `spaced-md.client/src/components/TopBar.vue` — wire up search input + dropdown
- `spaced-md.client/src/api/client.ts` — add search types
...
- Modify: `spaced-md.Server/Domain/Entities/Deck.cs`
- Modify: `spaced-md.Server/Domain/Entities/MarkdownFile.cs`
- Modify: `spaced-md.Server/Infrastructure/Data/AppDbContext.cs`
- Create: migration file (auto-generated)

- [ ] **Step 1: Add NpgsqlTsVector property to Card entity**

In `spaced-md.Server/Domain/Entities/Card.cs`, add:
```csharp
using NpgsqlTypes;
// ... existing properties ...
...
In `spaced-md.Server/Domain/Entities/Deck.cs`, add:
```csharp
using NpgsqlTypes;
// ... existing properties ...
public NpgsqlTsVector SearchVector { get; set; } = null!;
...
In `spaced-md.Server/Domain/Entities/MarkdownFile.cs`, add:
```csharp
using NpgsqlTypes;
// ... existing properties ...
public NpgsqlTsVector SearchVector { get; set; } = null!;
...
In `spaced-md.Server/Infrastructure/Data/AppDbContext.cs`, add to the `Card` entity config (inside `OnModelCreating`):
```csharp
...
Run:
```bash
cd spaced-md.Server && dotnet ef migrations add AddSearchVectors
```
...
Run:
```bash
docker exec -it spacedmd-postgres psql -U spaced -d spacedmd -c "\d \"Cards\"" | grep -i search
```
...
```bash
git add spaced-md.Server/Domain/Entities/Card.cs spaced-md.Server/Domain/Entities/Deck.cs spaced-md.Server/Domain/Entities/MarkdownFile.cs spaced-md.Server/Infrastructure/Data/AppDbContext.cs spaced-md.Server/Infrastructure/Data/Migrations/
...
**Files:**
- Create: `spaced-md.Server/Api/Endpoints/SearchEndpoints.cs`
- Modify: `spaced-md.Server/Program.cs:69` — add `app.MapSearchEndpoints();`
...
In `spaced-md.Server/Program.cs`, add after line 69 (`app.MapDeckEndpoints();`):
```csharp
...
```bash
git add spaced-md.Server/Api/Endpoints/SearchEndpoints.cs spaced-md.Server/Program.cs
...
**Files:**
- Modify: `spaced-md.client/src/api/client.ts` — add search types
- Create: `spaced-md.client/src/composables/useSearch.ts`

- [ ] **Step 1: Add search types to api/client.ts**

Append to `spaced-md.client/src/api/client.ts`:
```typescript
// Search types
...
```bash
git add spaced-md.client/src/api/client.ts spaced-md.client/src/composables/useSearch.ts
...
**Files:**
- Create: `spaced-md.client/src/components/SearchResults.vue`
...
```bash
git add spaced-md.client/src/components/SearchResults.vue
