...
**Goal:** Allow AI agents to interact with fasolt through an MCP server backed by personal access tokens, bulk card creation, file upsert, and paginated list endpoints.
...
### Backend (new)
- `fasolt.Server/Domain/Entities/ApiToken.cs` — token entity with hashed value, name, expiry
- `fasolt.Server/Api/Endpoints/ApiTokenEndpoints.cs` — CRUD endpoints for token management
- `fasolt.Server/Api/Auth/BearerTokenHandler.cs` — custom auth handler for Bearer token validation
- `fasolt.Server/Api/Middleware/ErrorResponseMiddleware.cs` — standardized error JSON responses
- `fasolt.Server/Application/Dtos/ApiTokenDtos.cs` — token request/response DTOs
- `fasolt.Server/Application/Dtos/BulkCardDtos.cs` — bulk card creation DTOs
- `fasolt.Server/Application/Dtos/PaginationDtos.cs` — cursor-based pagination wrapper
- Migration file (auto-generated) — ApiTokens table

### Backend (modify)
- `fasolt.Server/Infrastructure/Data/AppDbContext.cs` — add ApiTokens DbSet and config
- `fasolt.Server/Program.cs` — register Bearer auth handler, error middleware, token endpoints
- `fasolt.Server/Api/Endpoints/CardEndpoints.cs` — add `POST /api/cards/bulk`, add `deckId` filter, add cursor pagination to list
- `fasolt.Server/Api/Endpoints/FileEndpoints.cs` — modify upload to support upsert, add cursor pagination to list
- `fasolt.Server/fasolt.Server.csproj` — no new packages needed for backend (SHA-256 is in System.Security.Cryptography)

### MCP Server (new project)
- `fasolt.Mcp/fasolt.Mcp.csproj` — dotnet tool project with ModelContextProtocol dependency
- `fasolt.Mcp/Program.cs` — host setup with DI, HttpClient, stdio transport
- `fasolt.Mcp/Tools/CardTools.cs` — search_cards, list_cards, create_cards
- `fasolt.Mcp/Tools/DeckTools.cs` — list_decks, create_deck
- `fasolt.Mcp/Tools/FileTools.cs` — upload_file, list_files, get_file
- `fasolt.Mcp/ApiClient.cs` — typed HttpClient wrapper for fasolt API

### Frontend (modify)
- `fasolt.client/src/views/SettingsView.vue` — add API Tokens section
- `fasolt.client/src/api/client.ts` — add token CRUD API calls, PaginatedResponse type
- `fasolt.client/src/stores/cards.ts` — unwrap paginated response
- `fasolt.client/src/stores/files.ts` — unwrap paginated response
...
**Files:**
- Create: `fasolt.Server/Domain/Entities/ApiToken.cs`
...
In `fasolt.Server/Infrastructure/Data/AppDbContext.cs`, add:
```csharp
...
dotnet ef migrations add AddApiTokens --project fasolt.Server
...
```bash
git add fasolt.Server/Domain/Entities/ApiToken.cs fasolt.Server/Infrastructure/Data/AppDbContext.cs fasolt.Server/Infrastructure/Data/Migrations/
...
**Files:**
- Create: `fasolt.Server/Api/Auth/BearerTokenHandler.cs`
...
```bash
git add fasolt.Server/Api/Auth/BearerTokenHandler.cs fasolt.Server/Program.cs
...
**Files:**
- Create: `fasolt.Server/Application/Dtos/ApiTokenDtos.cs`
- Create: `fasolt.Server/Api/Endpoints/ApiTokenEndpoints.cs`
- Modify: `fasolt.Server/Program.cs`
...
In `fasolt.Server/Program.cs`, add with the other `Map*Endpoints` calls:
```csharp
...
Start the backend (`dotnet run --project fasolt.Server`), then:
```bash
...
```bash
git add fasolt.Server/Application/Dtos/ApiTokenDtos.cs fasolt.Server/Api/Endpoints/ApiTokenEndpoints.cs fasolt.Server/Program.cs
...
**Files:**
- Create: `fasolt.Server/Api/Middleware/ErrorResponseMiddleware.cs`
- Modify: `fasolt.Server/Program.cs`
...
In `fasolt.Server/Program.cs`, after `app.UseAuthorization()` and before endpoint mapping:
```csharp
...
```bash
git add fasolt.Server/Api/Middleware/ErrorResponseMiddleware.cs fasolt.Server/Program.cs
...
**Files:**
- Create: `fasolt.Server/Application/Dtos/PaginationDtos.cs`
...
```bash
git add fasolt.Server/Application/Dtos/PaginationDtos.cs
...
In `fasolt.client/src/api/client.ts`, add:
```typescript
...
In `fasolt.client/src/stores/cards.ts`, change `fetchCards` (line 14):
```typescript
// Before:
...
In `fasolt.client/src/stores/files.ts`, change `fetchFiles` (line 13):
```typescript
// Before:
...
```bash
cd /Users/phil/Projects/fasolt/fasolt.client && npm run build
```
...
```bash
git add fasolt.client/src/stores/cards.ts fasolt.client/src/stores/files.ts fasolt.client/src/api/client.ts
...
**Files:**
- Create: `fasolt.Server/Application/Dtos/BulkCardDtos.cs`
- Modify: `fasolt.Server/Api/Endpoints/CardEndpoints.cs`
...
```bash
git add fasolt.Server/Application/Dtos/BulkCardDtos.cs fasolt.Server/Api/Endpoints/CardEndpoints.cs
...
- Modify: `fasolt.Server/Application/Dtos/FileDtos.cs`

- [ ] **Step 1: Add upsert response DTO**

In `fasolt.Server/Application/Dtos/FileDtos.cs`, add:
```csharp
...
```bash
git add fasolt.Server/Api/Endpoints/FileEndpoints.cs fasolt.Server/Application/Dtos/FileDtos.cs
...
In `fasolt.client/src/api/client.ts`, add types and API functions:
...
```bash
git add fasolt.client/src/api/client.ts fasolt.client/src/views/SettingsView.vue
...
**Files:**
- Create: `fasolt.Mcp/fasolt.Mcp.csproj`
- Create: `fasolt.Mcp/Program.cs`
- Create: `fasolt.Mcp/ApiClient.cs`
- Modify: `fasolt.sln`
...
```bash
cd /Users/phil/Projects/fasolt
dotnet new console -n fasolt.Mcp -o fasolt.Mcp
dotnet sln add fasolt.Mcp/fasolt.Mcp.csproj
```

- [ ] **Step 2: Configure the project as a dotnet tool**

Replace `fasolt.Mcp/fasolt.Mcp.csproj`:
```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <RootNamespace>Fasolt.Mcp</RootNamespace>
    <PackAsTool>true</PackAsTool>
    <ToolCommandName>fasolt-mcp</ToolCommandName>
    <PackageId>fasolt-mcp</PackageId>
...
**Files:**
- Create: `fasolt.Mcp/Tools/CardTools.cs`
...
```bash
git add fasolt.Mcp/Tools/CardTools.cs
...
**Files:**
- Create: `fasolt.Mcp/Tools/DeckTools.cs`
...
```bash
git add fasolt.Mcp/Tools/DeckTools.cs
...
**Files:**
- Create: `fasolt.Mcp/Tools/FileTools.cs`
...
```bash
git add fasolt.Mcp/Tools/FileTools.cs
...
```bash
FASOLT_URL=http://localhost:5000 FASOLT_TOKEN="$TOKEN" \
...
| 11 | MCP server project scaffold | `fasolt.Mcp/` project |
